...
namespace Colyseus
{
 ...
 /// <summary>
        ...
        public string WebSocketEndpoint
        {
            get
            {
                if (string.IsNullOrEmpty(colyseusServerPort) || colyseusServerPort.Equals(("443")))
                {
                    return (useSecureProtocol ? "wss" : "ws") + "://" + colyseusServerAddress + "/metabus-v4";
                }

                return (useSecureProtocol ? "ws" : "ws") + "://" + colyseusServerAddress + ":" + colyseusServerPort+"/metabus-v4";
            }
        }
        ...
        public string WebRequestEndpoint
        {
            get
            {
                if (string.IsNullOrEmpty(colyseusServerPort) || colyseusServerPort.Equals(("443")))
                {
	                return (useSecureProtocol ? "https" : "http") + "://" + colyseusServerAddress + "/metabus-v4";
                }

                return (useSecureProtocol ? "http" : "http") + "://" + colyseusServerAddress + ":" + colyseusServerPort+"/metabus-v4";
            }
        }
